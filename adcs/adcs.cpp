...
uint16_t ADCS::readReg(uint16_t reg) {
    unsigned char cmd[4] = {'$', (unsigned char) (reg & 0x00FF), (unsigned char)((reg >> 8) | 0x80), ']'};
    unsigned char tlm[8] = {0x00};
    //printf("sending: ");
    //printHex(cmd, 4);
    //printf("\n");
    serialFlush(fd);
...
void ADCS::writeReg(uint16_t reg, uint16_t data) {
    unsigned char cmd[6] = {'$', (unsigned char) (reg & 0x00FF), (unsigned char)((reg >> 8) | 0x80), (unsigned char) (data & 0x00FF), (unsigned char) (data >> 8),']'};
    //printf("sending: ");
    //printHex(cmd, 6);
    //printf("\n");
    serialFlush(fd);
...
void ADCS::readAttitudeData(AttitudePacket &packet) {
    ...
    //printf("sending: ");
    for (int i=0; i<16; i++) {
        cmd[1] = (unsigned char) (cmds[i] & 0x00FF);
        cmd[2] = (unsigned char) ((cmds[i] >> 8) | 0x80);
        //printHex(cmd, 4);
        if (write(fd, cmd, 4) < 0) perror("failed to write");
    }
    //printf("\n");
    ...
    dh = *((uint16_t*) (tlm+101));
    dl = *((uint16_t*) (tlm+109));
    packet.time_frt_week = (dh << 16) | dl;
    dh = *((uint16_t*) (tlm+117));
    dl = *((uint16_t*) (tlm+125));
    packet.time_frt_milli = (dh << 16) | dl;
}
...
void ADCS::readData(unsigned char* buffer, int size) {
    int bytesRead = 0;
